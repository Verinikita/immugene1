...
fun_rich<- function(data_list, T.name){
  if(T.name == "TRX.nt") {
    immudata$mmm<- purrr::map(data_list, ~ tidyr::separate(.x, V.name, c("V.name", "cloneIG"), sep = "IG"))
    #2 Filtro por los NA de la columna cloneIG
    df_CDR3filter <- lapply(immudata$mmm, function(x) x[which(is.na(x[,"cloneIG"])==TRUE), ])
    #3 Elimino la columna cloneIG
    df_CDR3TRX <- map(df_CDR3filter, ~ dplyr::select(.x,-one_of(c("cloneIG"))))  #elimino la columna otros creada arriba para separar clonotypos
    #4 Sumo las filas para CDR3nt
    data_CDR3nt_sum<- map(df_CDR3TRX,  ~ dplyr::group_by(.x, CDR3.nt) %>% dplyr::summarize(cont=dplyr::n()))
   ...
   data_CDR3nt_sum<- map(df_CDR3filter,  ~ dplyr::group_by(.x, CDR3.nt) %>% dplyr::summarize(cont=dplyr::n()))
   ...
   immudata$m2 <- lapply(filter1, function(x) x[which(is.na(x[,"cloneIG"])==TRUE), ])
   ...
   da_CDR3aa_filterim<- purrr::map(immudata$m5,  ~ dplyr::group_by(.x, CDR3.aa) %>% dplyr::summarize(cont=dplyr::n())) #Tiene en cuenta los unique para cada secuencia CDR3aa si esta 2 veces n=2
   #5 Cambio el nombre de la columna 2 = count  por la lista de nombres
   data_CDR3aa <-
   ...
   da_CDR3aa_filterim<- purrr::map(immudata$m7,  ~ dplyr::group_by(.x, CDR3.aa) %>% dplyr::summarize(cont=dplyr::n())) #Tiene en cuenta los unique para cada secuencia CDR3aa si esta 2 veces n=2
   #5 Cambio el nombre de la columna 2 = count  por la lista de nombres
   data_CDR3aa <-
     lapply(names(da_CDR3aa_filterim), function(i){
       x <- da_CDR3aa_filterim[[ i ]]
