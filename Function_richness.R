...
fun_rich<- function(data_list, T.name){
  if(T.name == "TRX.nt") {
    ...
    data_CDR3nt_sum<- purrr::map(df_CDR3TRX,  ~ dplyr::group_by(.x, nSeqCDR3) %>% dplyr::summarize(cont=dplyr::n()))
    ...
    data_CDR3ntf <- purrr::reduce(data_CDR3nt, dplyr::full_join, by = "nSeqCDR3")
    ...
    data_CDR3nt_sum<- purrr::map(df_CDR3filter,  ~ dplyr::group_by(.x, nSeqCDR3) %>% dplyr::summarize(cont=dplyr::n()))
    ...
    da_CDR3aa_filterim<- purrr::map(m5,  ~ dplyr::group_by(.x, aaSeqCDR3) %>% dplyr::summarize(cont=dplyr::n())) #Tiene en cuenta los unique para cada secuencia CDR3aa si esta 2 veces n=2
    #5 Cambio el nombre de la columna 2 = count  por la lista de nombres
    data_CDR3aa <-
    ...
    #6 Une los dataframes CDR3
    data_CDR3aaf <- purrr::reduce(data_CDR3aa, dplyr::full_join, by = "aaSeqCDR3")
    #7 Sumo las cantidades
    ...
    da_CDR3aa_filterim<- purrr::map(m7,  ~ dplyr::group_by(.x, aaSeqCDR3) %>% dplyr::summarize(cont=dplyr::n())) #Tiene en cuenta los unique para cada secuencia CDR3aa si esta 2 veces n=2
    #5 Cambio el nombre de la columna 2 = count  por la lista de nombres
    data_CDR3aa <-
      lapply(names(da_CDR3aa_filterim), function(i){
        x <- da_CDR3aa_filterim[[ i ]]
